...
import type { Exchange, ExchangeData, FilterOptions, SortDirection, SortField } from "@/types/exchange"
...
export function ExchangeList() {
        ...
        if (docSnapshot.exists()) {
          ...
          if (data.exchanges) {
            const countries = Array.from(
              new Set(
                data.exchanges.filter((exchange) => exchange.country).map((exchange) => exchange.country as string),
              ),
            ).sort()
            ...
            const years = Array.from(
              new Set(
                data.exchanges
                  .filter((exchange) => exchange.year_established)
                  .map((exchange) => exchange.year_established as number),
              ),
    ...
    if (filters.search) {
      ...
      filtered = filtered.filter(
        (exchange) =>
          ...
          (exchange.country && exchange.country.toLowerCase().includes(searchTerm)),
      )
    }

    // Apply trust score filter
    if (filters.minTrustScore > 0) {
      filtered = filtered.filter((exchange) => exchange.trust_score >= filters.minTrustScore)
    }

    // Apply country filter
    if (filters.country) {
      filtered = filtered.filter((exchange) => exchange.country === filters.country)
    }

    // Apply year established filter
    if (filters.yearEstablished) {
      filtered = filtered.filter((exchange) => exchange.year_established === filters.yearEstablished)
    }
  ...
  const totalPages = Math.ceil((displayedExchanges?.length || 0) / itemsPerPage)
  const paginatedExchanges = displayedExchanges.slice((currentPage - 1) * itemsPerPage, currentPage * itemsPerPage)
  ...
  const handlePageChange = (page: number) => {
    ...
    document.getElementById("exchange-table-top")?.scrollIntoView({ behavior: "smooth" })
  }
    ...
    <Card variant="bitcoin" className="w-full mx-auto">
      <CardHeader className="pb-2">
        <div className="flex flex-col md:flex-row md:items-center justify-end gap-4">
          {exchangeData && (
            <div className="text-xs text-muted-foreground">Last updated: {formatDate(exchangeData.last_updated)}</div>
          )}
        </div>
      </CardHeader>
      <CardContent>
        ...
        <div id="exchange-table-top" className="mt-6">
          <ExchangeTable
