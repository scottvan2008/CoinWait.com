...
export function PriceModelChart({
    ...
    const yDomain = useMemo(() => {
        ...
        const modelPrices = currentYearData.map((d) => d.modelPrice);
        const actualPrices = currentYearData
            .filter((d) => d.actualPrice !== undefined)
            .map((d) => d.actualPrice as number);
        ...
        <div className="h-[400px] w-full">
            {isFutureYear && (
                <div className="text-sm text-center text-amber-600 dark:text-amber-400 mb-2">
                    Future year: Only model prices are shown (no actual price
                    data available)
                </div>
            )}
