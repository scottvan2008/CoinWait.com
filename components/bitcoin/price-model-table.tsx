...
export function PriceModelTable({
    data,
    isFutureYear = false,
}: PriceModelTableProps) {
    const [currentPage, setCurrentPage] = useState(1);
    const [itemsPerPage, setItemsPerPage] = useState(30);
    const isMobile = useIsMobile();

    // Memoize pagination calculations
    const { totalItems, totalPages, startIndex, paginatedData } =
        useMemo(() => {
            // Filter data to ensure we're only using data from the current year
            const currentYearData =
                data.length > 0
                    ? data.filter((item) => {
                          const year = item.date.split("-")[0];
                          return year === data[0].date.split("-")[0]; // Use the year from the first data point
                      })
                    : [];
    ...
    const summaryMetrics = useMemo(() => {
        ...
        paginatedData.forEach((item) => {
            if (item.actualPrice) {
                const diff =
                    ((item.actualPrice - item.modelPrice) / item.modelPrice) *
                    100;
    ...
    const handlePageChange = useCallback(
        (page: number) => {
            ...
            document
                .getElementById("price-model-table-top")
    ...
    const getTrendIcon = (difference: number | null) => {
        ...
        if (difference > 10)
            return (
                <ArrowUp className="h-4 w-4 text-green-500 dark:text-green-500" />
            );
        if (difference < -20)
            return (
        ...
        if (difference < -10)
            return (
                <ArrowDown className="h-4 w-4 text-red-500 dark:text-red-500" />
            );
                ...
                <div className="p-4 mb-4 bg-gradient-to-r from-amber-50 to-amber-100/70 dark:from-amber-900/20 dark:to-amber-800/10 border border-amber-200 dark:border-amber-800/30 rounded-lg text-amber-800 dark:text-amber-300 text-sm shadow-sm">
                    <div className="flex items-start gap-3">
                        ...
                        <div>
                            ...
                            This is a future year showing only model prices
                            based on the mathematical formula. No actual price
                            data is available.
                        </div>
                    </div>
                </div>
                            ...
                            <div>
                                ...
                                >
                                    {summaryMetrics.averageDifference > 0
                                        ? "+"
                                        : ""}
                                    {summaryMetrics.averageDifference.toFixed(
                                        2
                                    )}
                                    %
                                </p>
                        ...
                        <CardContent className="p-3 flex items-center justify-between">
                            <div>
                                ...
                                <p className="text-lg font-bold text-green-600 dark:text-green-400">
                                    {summaryMetrics.overvaluedCount}
                                </p>
                            </div>
                        ...
                        <CardContent className="p-3 flex items-center justify-between">
                            <div>
                                ...
                                <p className="text-lg font-bold text-red-600 dark:text-red-400">
                                    {summaryMetrics.undervaluedCount}
                                </p>
                            </div>
                            ...
                            {paginatedData.map((item, index) => {
                                ...
                                return (
                                    ...
                                    >
                                        ...
                                        <td
                                            className={`border-b border-gray-100 dark:border-gray-800 p-2 text-center ${getDifferenceBackground(
                                                difference
                                            )}`}
                                        >
                                            {difference !== null ? (
                                                <div
                                                    className={`flex items-center justify-center gap-1 ${getDifferenceClass(
                                                        difference
                                                    )}`}
                                                >
