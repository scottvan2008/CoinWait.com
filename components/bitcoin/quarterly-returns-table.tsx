...
export function QuarterlyReturnsTable() {
    ...
    const [averageReturns, setAverageReturns] = useState<
    ...
    useEffect(() => {
        async function fetchData() {
            try {
                ...
                setAverageReturns(
                    calculateAverageReturns(processedData.quarterlyReturns)
                );
        ...
        yearsData.forEach((yearData) => {
            ...
            QUARTERS.forEach((quarter) => {
                ...
                const firstMonthWithData = quarter.months.find(
                    (month) =>
                        monthlyPrices[month] && monthlyPrices[month].length > 0
                );
                ...
                const lastMonthWithData = [...quarter.months]
                    ...
                    .find(
                        (month) =>
                            monthlyPrices[month] &&
                            monthlyPrices[month].length > 0
                    );

                // If we don't have data for any month in the quarter, return null
                if (
                    firstMonthWithData === undefined ||
                    lastMonthWithData === undefined
                ) {
                ...
                const lastPrice =
                    monthlyPrices[lastMonthWithData][
                        monthlyPrices[lastMonthWithData].length - 1
                    ];
    ...
    function calculateAverageReturns(
        quarterlyReturns: Record<string, Record<string, number | null>>
    ) {
