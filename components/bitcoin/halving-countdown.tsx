          ...
          <div className="grid grid-cols-4 gap-4 md:gap-8 text-center px-2 md:px-8">
            {[
              ...
              <div key={index} className="flex flex-col">
                ...
                <span className="bitcoin-countdown-label-large dark:text-gray-300">{item.label}</span>
              </div>
            ))}
          </div>

          {/* Estimated Date */}
          <div className="flex items-center justify-center gap-2 text-base md:text-lg text-muted-foreground bg-bitcoin-background dark:bg-gray-800 p-4 md:p-5 rounded-lg mx-2 md:mx-8">
            <Clock className="h-6 w-6 text-bitcoin" />
            <span className="dark:text-gray-300">
              Estimated halving date: {blockData ? formatDate(blockData.estimated_halving_time) : "-"}
            </span>
          </div>
        </CardContent>
      </Card>
