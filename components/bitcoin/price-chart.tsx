...
export function PriceChart({ year, month, priceData }: PriceChartProps) {
    const [chartData, setChartData] = useState<
        Array<{ date: string; day: number; price: number | null }>
    >([]);
    ...
    // Update only the formatXAxis function to show every 5th day on mobile
    const formatXAxis = (day: number) => {
        // For mobile, only show every 5th day to prevent overcrowding
        if (isMobile) {
            return day % 5 === 0 ? day.toString() : "";
        }
        // For desktop, show every 3rd day
