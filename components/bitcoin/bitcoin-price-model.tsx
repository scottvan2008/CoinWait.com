...
export function BitcoinPriceModel() {
        ...
        async function fetchData() {
            try {
                ...
                querySnapshot.forEach((doc) => {
                    ...
                    if (
                        !latestUpdate ||
                        new Date(data.last_updated) > new Date(latestUpdate)
                    ) {
    ...
    const yearData = useMemo(() => {
        ...
        for (
            ...
            // Ensure the date belongs to the selected year
            if (dateString.startsWith(selectedYear)) {
                ...
                {loading ? (
                    ...
                    <div className="p-4 text-red-500 dark:text-red-400">
                        ...
                        <p className="mt-2 text-sm text-muted-foreground">
                            Please check your Firestore configuration and try
                            again.
                        </p>
                    </div>
                ) : (
                    <div>
                        ...
                        <div className="p-4 border-b border-gray-200 dark:border-gray-700">
                            <h3 className="text-lg font-medium mb-4 text-bitcoin-dark dark:text-white">
                                Price Model Chart
                            </h3>
                            <PriceModelChart
                                data={yearData}
                        ...
                        <div className="px-4 py-3 bg-bitcoin-background/30 dark:bg-gray-800/30 text-xs text-muted-foreground border-t border-gray-200 dark:border-gray-700">
                            <p>
                                <strong>About ahr999's model:</strong> This
                                logarithmic growth model suggests that Bitcoin's
                                price follows a predictable pattern as the
                                network matures. Positive differences indicate
                                that Bitcoin is trading above the model's price
                                (potentially overvalued), while negative
                                differences suggest it may be undervalued
                                according to the model. For future years, the
                                model provides price projections based on the
