...
import { formatCurrency, formatNumber, formatDate, formatShortDate, formatCryptoPrice } from "@/lib/formatters"
  ...
  return (
    <Dialog open={isOpen} onOpenChange={(open) => !open && onClose()}>
      <DialogContent className="max-w-3xl max-h-[90vh] overflow-y-auto p-4 sm:p-6 w-[95vw] sm:w-auto">
        <DialogHeader className={`flex ${isMobile ? "flex-col items-start" : "flex-row items-center"} gap-3`}>
          ...
          <div>
            <DialogTitle className={`${isMobile ? "text-xl" : "text-2xl"} flex items-center flex-wrap gap-1 sm:gap-2`}>
              {coin.name}
              <span className="text-xs sm:text-sm text-muted-foreground uppercase">{coin.symbol}</span>
        ...
        <div className="mt-4">
          <Tabs defaultValue="overview" className="w-full">
            <TabsList className="grid grid-cols-3 mb-4 sm:mb-6 bg-bitcoin-background dark:bg-gray-800 text-xs sm:text-sm">
              <TabsTrigger value="overview" className="data-[state=active]:bg-bitcoin data-[state=active]:text-white">
                <LineChart className="h-3 w-3 sm:h-4 sm:w-4 mr-1 sm:mr-2" />
                <span className={isMobile ? "text-xs" : ""}>Overview</span>
              </TabsTrigger>
              <TabsTrigger value="market" className="data-[state=active]:bg-bitcoin data-[state=active]:text-white">
                <DollarSign className="h-3 w-3 sm:h-4 sm:w-4 mr-1 sm:mr-2" />
                <span className={isMobile ? "text-xs" : ""}>Market</span>
              </TabsTrigger>
              <TabsTrigger value="details" className="data-[state=active]:bg-bitcoin data-[state=active]:text-white">
            ...
            <TabsContent value="overview" className="space-y-6">
              <div className="flex flex-col sm:flex-row justify-between gap-4 p-3 sm:p-4 bg-bitcoin-background dark:bg-gray-800 rounded-lg">
                <div>
                ...
                <div>
                  ...
                  <div className="flex items-center gap-2 mt-1">
                    <span className="text-sm font-medium">{formatCryptoPrice(coin.low_24h)}</span>
                    <div className="h-1 w-20 bg-gray-200 dark:bg-gray-700 rounded-full relative">
                      <div
                        className="absolute h-3 w-3 bg-bitcoin rounded-full top-1/2 transform -translate-y-1/2"
                        style={{
                          left: `${((coin.current_price - coin.low_24h) / (coin.high_24h - coin.low_24h)) * 100}%`,
                        }}
                      />
                    </div>
                    <span className="text-sm font-medium">{formatCryptoPrice(coin.high_24h)}</span>
                  </div>
                </div>
                ...
                <div>
                  ...
                  >
                    ...
                    {Math.abs(coin.market_cap_change_percentage_24h).toFixed(2)}% (24h)
                  </div>
                </div>
              </div>
              ...
              <div className="space-y-2">
                <div className="flex justify-between items-center">
                  <div className="flex items-center">
                    ...
                    <span className="text-sm font-medium">Price Range (All Time)</span>
                  </div>
                </div>
                <div className="space-y-2 p-4 bg-bitcoin-background dark:bg-gray-800 rounded-lg">
                  <div className="flex justify-between text-xs text-muted-foreground">
                    <div>
                      ...
                      <div className="text-xs">{formatShortDate(coin.atl_date)}</div>
                    </div>
                    <div className="text-right">
                      ...
                      <div className="text-xs">{formatShortDate(coin.ath_date)}</div>
                    </div>
                  </div>
                  ...
                  <div className="mt-2 text-center">
                    <div className="text-xs text-muted-foreground">
                      Current: {formatCryptoPrice(coin.current_price)}
                    </div>
                    <div className="text-xs text-muted-foreground">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div className="bitcoin-stat-card">
                  ...
                  <div className="text-lg font-medium text-bitcoin-dark dark:text-white">
                    {formatNumber(coin.circulating_supply, 0)} {coin.symbol.toUpperCase()}
                  </div>
                  {coin.max_supply && (
                    <div className="mt-2">
                ...
                <div className="bitcoin-stat-card">
                  ...
                  <div className="text-lg font-medium text-bitcoin-dark dark:text-white">
                    {formatNumber(coin.total_supply, 0)} {coin.symbol.toUpperCase()}
                  </div>
                </div>
                <div className="bitcoin-stat-card">
                  <div className="text-sm text-muted-foreground">Max Supply</div>
                  <div className="text-lg font-medium text-bitcoin-dark dark:text-white">
                    {coin.max_supply ? formatNumber(coin.max_supply, 0) : "∞"} {coin.symbol.toUpperCase()}
                  </div>
                </div>
              </div>
            </TabsContent>
            ...
            <TabsContent value="market" className="space-y-4">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="bitcoin-stat-card">
                  ...
                  <div className="text-xl font-medium text-bitcoin-dark dark:text-white">#{coin.market_cap_rank}</div>
                </div>
                <div className="bitcoin-stat-card">
                  <div className="text-sm text-muted-foreground">Market Cap</div>
                  ...
                  >
                    ...
                    {formatCurrency(Math.abs(coin.market_cap_change_24h), 0)} (
                    {Math.abs(coin.market_cap_change_percentage_24h).toFixed(2)}%) (24h)
                  </div>
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="bitcoin-stat-card">
                  ...
                  <div className="text-xs text-muted-foreground mt-1">
                    {((coin.total_volume / coin.market_cap) * 100).toFixed(2)}% of market cap
                  </div>
                </div>
                <div className="bitcoin-stat-card">
                  <div className="text-sm text-muted-foreground">Fully Diluted Valuation</div>
                  <div className="text-xl font-medium text-bitcoin-dark dark:text-white">
                    {coin.fully_diluted_valuation ? formatCurrency(coin.fully_diluted_valuation, 0) : "N/A"}
                  </div>
                </div>
              </div>
              ...
              <div className="bitcoin-stat-card">
                <div className="text-sm text-muted-foreground mb-2">Price Change</div>
                <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
                  {[
                    { label: "1 Hour", value: coin.price_change_percentage_1h_in_currency },
                    { label: "24 Hours", value: coin.price_change_percentage_24h },
                    { label: "7 Days", value: coin.price_change_percentage_7d_in_currency },
                    { label: "14 Days", value: coin.price_change_percentage_14d_in_currency },
                    { label: "30 Days", value: coin.price_change_percentage_30d_in_currency },
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="bitcoin-stat-card">
                  ...
                  >
                    {coin.price_change_24h > 0 ? "+" : ""}
                    {formatCurrency(coin.price_change_24h)}({coin.price_change_percentage_24h > 0 ? "+" : ""}
              ...
              {coin.roi && (
                <div className="bitcoin-stat-card">
                  ...
                  <div
                    className={`text-xl font-medium ${
                      coin.roi.percentage > 0 ? "text-green-600 dark:text-green-400" : "text-red-600 dark:text-red-400"
                    }`}
                  >
                    {coin.roi.percentage > 0 ? "+" : ""}
                    {coin.roi.percentage.toFixed(2)}%
                  </div>
                  <div className="text-xs text-muted-foreground mt-1">Currency: {coin.roi.currency.toUpperCase()}</div>
                </div>
              )}
            </TabsContent>
            ...
            <TabsContent value="details" className="space-y-4">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="bitcoin-stat-card">
                  <div className="flex items-center gap-2">
                    ...
                    <div className="text-sm text-muted-foreground">All Time Low Date</div>
                  </div>
                  <div className="text-base font-medium text-bitcoin-dark dark:text-white mt-1">
                    {formatShortDate(coin.atl_date)}
                  </div>
                  <div className="text-xs text-muted-foreground">{formatDate(coin.atl_date)}</div>
                  <div className="text-sm font-medium mt-1">Price: {formatCryptoPrice(coin.atl)}</div>
                ...
                <div className="bitcoin-stat-card">
                  <div className="flex items-center gap-2">
                    <Calendar className="h-4 w-4 text-muted-foreground" />
                    <div className="text-sm text-muted-foreground">All Time High Date</div>
                  </div>
                  <div className="text-base font-medium text-bitcoin-dark dark:text-white mt-1">
                    {formatShortDate(coin.ath_date)}
                  </div>
                  <div className="text-xs text-muted-foreground">{formatDate(coin.ath_date)}</div>
                  <div className="text-sm font-medium mt-1">Price: {formatCryptoPrice(coin.ath)}</div>
                  <div
                    className={`text-xs ${
                      coin.ath_change_percentage > 0
                        ? "text-green-600 dark:text-green-400"
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                ...
                <div className="bitcoin-stat-card">
                  ...
                  <div className="text-base font-medium text-bitcoin-dark dark:text-white mt-1">{coin.name}</div>
                </div>
              </div>
            </TabsContent>
          </Tabs>
