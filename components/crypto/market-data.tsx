...
import type { CryptoData, MarketData, TimeFrame } from "@/types/crypto"
import { formatCurrency, formatDate, formatCryptoPrice } from "@/lib/formatters"
...
export function CryptoMarketData({ timeFrame = "24h" }: CryptoMarketDataProps) {
  ...
  const getPriceChangePercentage = (coin: CryptoData) => {
    switch (timeFrame) {
      case "7d":
        ...
        <CardContent>
          <Tabs defaultValue="all" className="w-full" onValueChange={setActiveTab}>
            ...
            <div id="crypto-table-top" className="overflow-x-auto">
              <table
                className={`w-full ${isMobile ? "min-w-[320px]" : "min-w-[640px]"} table-auto ${isMobile ? "text-sm" : ""}`}
              >
                <thead>
                  <tr className="border-b border-gray-200 dark:border-gray-700">
                    <th
                      className={`${isMobile ? "px-1" : "px-4"} py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider`}
                    >
                      #
                    </th>
                    <th
                    ...
                    <th
                      className={`${isMobile ? "px-1" : "px-4"} py-3 text-right text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider`}
                    >
                      Price
                    </th>
                    <th
                      ...
                      <td className="px-2 sm:px-4 py-3 sm:py-4 whitespace-nowrap">
                        <div className="flex items-center">
                          ...
                          <div className="ml-2 sm:ml-4">
                            ...
                            {!isMobile && <div className="text-xs text-gray-500 dark:text-gray-400">{coin.name}</div>}
                          </div>
                        </div>
                      </td>
                      <td
                        className={`${isMobile ? "px-1" : "px-4"} py-3 whitespace-nowrap text-sm text-right font-medium text-gray-900 dark:text-white`}
                      >
                        {formatCryptoPrice(coin.current_price)}
                      </td>
                      <td className={`${isMobile ? "px-1" : "px-4"} py-3 whitespace-nowrap text-sm text-right`}>
                        <div
                          className={`flex items-center justify-end ${
                            getPriceChangePercentage(coin) > 0
                        ...
                        >
                          {getPriceChangePercentage(coin) > 0 ? (
                            <ChevronUp className={`${isMobile ? "h-3 w-3 mr-0.5" : "h-4 w-4 mr-1"}`} />
                          ) : getPriceChangePercentage(coin) < 0 ? (
                            <ChevronDown className={`${isMobile ? "h-3 w-3 mr-0.5" : "h-4 w-4 mr-1"}`} />
                          ) : (
                            <ChevronsUpDown className={`${isMobile ? "h-3 w-3 mr-0.5" : "h-4 w-4 mr-1"}`} />
                          )}
                          {Math.abs(getPriceChangePercentage(coin)).toFixed(2)}%
                        </div>
                      </td>
                      {!isMobile && (
                        <>
