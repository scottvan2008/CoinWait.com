...
import { XAxis, YAxis, CartesianGrid, Tooltip, ResponsiveContainer, ReferenceArea, Area, AreaChart } from "recharts"

export function FearGreedIndex() {
  const [fearGreedData, setFearGreedData] = useState<FearGreedData | null>(null)
  const [historicalData, setHistoricalData] = useState<FearGreedHistoricalData | null>(null)
  ...
  useEffect(() => {
    async function fetchData() {
      try {
        ...
        const historicalResponse = await fetch("https://api.alternative.me/fng/?limit=30")
  ...
  const classificationInfo: FearGreedClassificationInfo[] = [
    {
      name: "Extreme Fear",
      description: "Investors are in extreme fear and prices may be oversold. This can be a buying opportunity.",
      color: "text-pink-500 dark:text-pink-400",
      range: [0, 25],
    },
    {
      name: "Fear",
      description: "Investors are fearful, which may indicate a market that is becoming nervous.",
      color: "text-orange-500 dark:text-orange-400",
      range: [26, 40],
    },
    {
      name: "Neutral",
      description: "Investors are neutral with no strong bias toward fear or greed.",
      color: "text-yellow-500 dark:text-yellow-400",
      range: [41, 60],
    },
    {
      name: "Greed",
      description: "Investors are becoming greedy, possibly indicating a market that is heating up.",
      color: "text-lime-500 dark:text-lime-400",
      range: [61, 75],
    },
    {
      name: "Extreme Greed",
      description: "Investors are extremely greedy and prices may be overvalued. This can be a selling opportunity.",
    ...
    if (active && payload && payload.length) {
      ...
      return (
        <div className="bg-white dark:bg-gray-800 p-3 border border-gray-200 dark:border-gray-700 rounded-lg shadow-lg">
          <p className="text-sm font-medium text-gray-600 dark:text-gray-300">{data.fullDate}</p>
          <p className={`text-lg font-bold ${getFearGreedColor(data.value)}`}>
          ...
          <div className="mt-8">
            ...
            <Tabs defaultValue="week" onValueChange={setTimeFrame}>
              <TabsList className="grid grid-cols-3 mb-4 bg-bitcoin-background dark:bg-gray-800">
                ...
                <TabsTrigger value="2week" className="data-[state=active]:bg-bitcoin data-[state=active]:text-white">
                  14 Days
                </TabsTrigger>
                <TabsTrigger value="month" className="data-[state=active]:bg-bitcoin data-[state=active]:text-white">
                  30 Days
                </TabsTrigger>
              </TabsList>

              <TabsContent value={timeFrame} className="mt-4">
                <div className="bg-white dark:bg-gray-900 p-4 rounded-lg shadow-md border border-bitcoin/20 dark:border-bitcoin/10">
                  <div className="h-[400px] w-full">
                    <ResponsiveContainer width="100%" height="100%">
                      <AreaChart data={getChartData()} margin={{ top: 20, right: 20, left: 10, bottom: 20 }}>
                        <defs>
                        ...
                        <ReferenceArea y1={0} y2={25} fill="rgba(250, 90, 125, 0.05)" />
                        <ReferenceArea y1={25} y2={40} fill="rgba(255, 159, 69, 0.05)" />
                        <ReferenceArea y1={40} y2={60} fill="rgba(255, 209, 102, 0.05)" />
                        <ReferenceArea y1={60} y2={75} fill="rgba(155, 227, 109, 0.05)" />
                        <ReferenceArea y1={75} y2={100} fill="rgba(60, 216, 86, 0.05)" />
                          ...
                          dot={(props) => {
                            ...
                            return (
                              <circle
                                ...
                                stroke="white"
                                strokeWidth={2}
                                fill={getFearGreedHexColor(value)}
                    ...
                    {classificationInfo.map((info, index) => (
                      ...
                      >
                        <div
                          ...
                          style={{ backgroundColor: getFearGreedHexColor((info.range[0] + info.range[1]) / 2) }}
                        />
                        <span className={`text-xs font-medium ${info.color}`}>
                          {info.name} ({info.range[0]}-{info.range[1]})
                        </span>
                      </div>
                    ))}
                  </div>
                  ...
                  <div className="flex items-start gap-4 mb-6">
                    ...
                    <div>
                      ...
                      <p className="text-gray-600 dark:text-gray-300 text-sm sm:text-base">
                        A tool that measures market sentiment from 0 (Extreme Fear) to 100 (Extreme Greed)
                      </p>
                    </div>
                  </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                      <div className="bg-gray-50 dark:bg-gray-800 p-5 rounded-xl">
                        ...
                        <p className="text-gray-600 dark:text-gray-300 text-sm leading-relaxed">
                          The index analyzes emotions and sentiments from different sources including price volatility,
                          market momentum, social media, surveys, and trading volume to generate a single number
                          representing the current market sentiment.
                        </p>
                      </div>
                      ...
                      <div className="bg-gray-50 dark:bg-gray-800 p-5 rounded-xl">
                        ...
                        <p className="text-gray-600 dark:text-gray-300 text-sm leading-relaxed">
                          Extreme fear can indicate buying opportunities as investors are too worried. When investors
                          get too greedy, the market might be due for a correction. The index helps identify potential
                    ...
                    <div className="bg-gray-50 dark:bg-gray-800 p-5 rounded-xl">
                      ...
                      <div className="relative h-12 bg-gradient-to-r from-[#FA5A7D] via-[#FFD166] to-[#3CD856] rounded-lg mb-4">
                        ...
                        <div className="absolute -bottom-8 inset-x-0 flex justify-between px-0">
                          ...
                          <div className="text-[#FF9F45] text-xs font-medium">Fear</div>
                          <div className="text-[#FFD166] text-xs font-medium">Neutral</div>
                          <div className="text-[#9BE36D] text-xs font-medium">Greed</div>
                      ...
                      <div className="mt-12 pt-2 text-sm text-gray-500 dark:text-gray-400 flex items-start gap-2 border-t border-gray-200 dark:border-gray-700">
                        ...
                        <p className="italic">
                          Remember: This index is not financial advice. Always do your own research before making
